...
<html lang="<?= ACTIVE_LANG ?>">
    ...
    <body>
        <?php 
            $Nav = new stdClass;
            $Nav->activeMenu = "schedule-calendar";
            require_once(APPPATH.'/views/fragments/navigation.fragment.php');
        ?>

        <?php 
            $TopBar = new stdClass;
            $TopBar->title = __("Scheduled Posts");
            if ($viewtype == "day") {
                $date = new \Moment\Moment("$year-$month-$day 00:00:00", 
        ...
        <?php require_once(APPPATH.'/views/fragments/schedule-calendar-'.($viewtype == "day" ? "day" : "month").'.fragment.php'); ?>
