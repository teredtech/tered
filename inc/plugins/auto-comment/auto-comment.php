...
function install($Plugin)
{
    ...
    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_comment_schedule` 
                ADD CONSTRAINT `acschdl_ibfk_1` FOREIGN KEY (`user_id`) 
                REFERENCES `".TABLE_PREFIX."users`(`id`) 
                ON DELETE CASCADE ON UPDATE CASCADE;";

    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_comment_schedule` 
                ADD CONSTRAINT `acschdl_ibfk_2` FOREIGN KEY (`account_id`) 
    ...
    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_comment_log` 
                ADD CONSTRAINT `aclg_ibfk_1` FOREIGN KEY (`user_id`) 
                REFERENCES `".TABLE_PREFIX."users`(`id`) 
                ON DELETE CASCADE ON UPDATE CASCADE;";

    $sql .= "ALTER TABLE `".TABLE_PREFIX."auto_comment_log` 
                ADD CONSTRAINT `aclg_ibfk_2` FOREIGN KEY (`account_id`) 
                REFERENCES `".TABLE_PREFIX."accounts`(`id`) 
                ON DELETE CASCADE ON UPDATE CASCADE;";
...
function addCronTask()
{
    ...
    foreach ($Schedules->getDataAs($as) as $sc) {
        ...
        // Select random comment from the defined comment collection
        ...
        if (empty($media_id)) {
            $Log->set("data", "Couldn't find the new media to comment")->save();
            continue;
        }
        ...
        // Like it!
        try {
            $res = $Instagram->media->comment($media_id, $comment);
        } catch (\Exception $e) {
            $Log->set("data", $e->getMessage())->save();
            continue;
        }
